        ...
        if self.chunked_prefill_size is None and int(envs.ENABLE_V1_KVCACHE_SCHEDULER):
            if current_platform.is_cuda():
                try:
                    ...
                    import logging
                    logging.getLogger("fastdeploy").info(f"Auto-detected GPU memory: {gpu_mem:.1f}GB, chunked_prefill_size: {self.chunked_prefill_size}")
                except Exception as e:
                    import logging
                    logging.getLogger("fastdeploy").warning(f"Failed to detect GPU memory, using default chunked_prefill_size: {e}")
